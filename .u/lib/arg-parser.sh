...
_lib__arg_parser()
{
    ...
    eval "$(
        ...
        then
            ...
            _print_err "No arguments passed to \`_lib__arg_parser\`."
        ...
        do
            ...
            # printf "%s\n" "printf \">1 %s\n\" \"${1}\"" >&3
            _parser_args="${_parser_args} \"${1}\""
            # printf "%s\n" "printf \">2 %s\n\" \"${_parser_args}\"" >&3

            shift
        done

        # printf "%s\n" "echo \"_parser_args: ${_parser_args}\"" >&3

        shift

        # # Determine remainder var name if arg parser is configured to calculate remainder script arguments that are beyond the reach of the final array, or the `--` delimiter.
        ...
        # # If the parser is configured to get the remainder, get the last flag for remainder calculation whilst calculating flag values.
        # if
        ...
        do
            ...
            then
                ...
                test "${_next_stage}" = "GET_VAR_NAME"
            then
                # TODO: Validate `_var_name` for illegal characters.

                _var_name="${_parser_arg}"

                # Ensure the variable is empty in the parent shell to allow for easy use of this lib.
                # TODO: Remove this if each variable specified in the lib function's arguments is always set to a value after successful completion of the lib function.
                printf "%s\n" "${_parser_arg}=\"\"" >&3
                ...
                test "${_next_stage}" = "GET_ARRAY_LENGTH"
            then
                    _array_length="${_parser_arg}"

                    # If `_array_length` not `n` or a digit. Validation. 
                    if
                        test -n "$(printf "%s" "${_array_length}" | sed -n "/[n\\d]/p")"
                    then
                ...
                do
                    ...
                    do
                        ...
                        if
                            ...
                            && test -n "$(printf "%s" "${_arg}" | sed -n "/^-/p")"
                        then
                            # If the length of the array has not been exceeded, including if `_count` is `n`. 
                            if
                                test "${_count}" = "n" \
                                || test "${_count}" -gt 0
                            then
            ...
            printf "%s\n" "echo \"_parser_arg is \`${_parser_arg}\`\"" >&3

            if
                test "${_parser_arg}" = "--bool"
            then
                # debug
                printf "%s\n" "echo \"parser arg is \`--bool\`\"" >&3

                _var_type="BOOL"
                _next_stage="GET_VAR_NAME"
            elif
                test "${_parser_arg}" = "--array"
            then
                _var_type="ARRAY"
                _next_stage="GET_VAR_NAME"
            fi
